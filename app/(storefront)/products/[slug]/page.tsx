...
export default async function ProductPage({ params }: ProductPageProps) {
  ...
  // Compute compareAtPrice as a dynamic fallback (column not yet in production DB)
  const prices = product.variants.map((v) => Number(v.price));
  const minPrice = prices.length > 0 ? Math.min(...prices) : Number(product.basePrice);
  const compareAtPrice = Math.round(minPrice * 1.2); // 20% markup as "compare at" fallback
