...
export function AbandonedCartsManager({ carts }: { carts: any[] }) {
  ...
  const getStatusBadge = (status: string) => {
    switch (status.toLowerCase()) {
      case "recovered":
        return <span className="bg-emerald-100 text-emerald-700 px-2.5 py-0.5 rounded-full text-xs font-bold border border-emerald-200 flex items-center gap-1 w-max"><CheckCircle className="w-3 h-3" /> Recovered</span>;
      case "lost":
        return <span className="bg-rose-100 text-rose-700 px-2.5 py-0.5 rounded-full text-xs font-bold border border-rose-200 flex items-center gap-1 w-max"><XCircle className="w-3 h-3" /> Lost</span>;
      default:
        return <span className="bg-amber-100 text-amber-700 px-2.5 py-0.5 rounded-full text-xs font-bold border border-amber-200 flex items-center gap-1 w-max"><Clock className="w-3 h-3" /> Pending</span>;
    }
  };
                ...
                <tr key={cart.id} className="hover:bg-gray-50/50 transition-colors">
                  <td className="px-6 py-4">
                    ...
                    <p className="text-xs text-gray-500 mt-1 flex gap-2">
                      {cart.guestEmail && <span className="flex items-center gap-1"><Mail className="w-3 h-3" />{cart.guestEmail}</span>}
                      {cart.user?.phone && <span className="flex items-center gap-1"><MessageCircle className="w-3 h-3" />{cart.user.phone}</span>}
                    </p>
                  </td>
          ...
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-lg overflow-hidden border border-gray-100">
            <div className="flex items-center justify-between p-5 border-b border-gray-100">
              <div>
                ...
                <p className="text-xs text-gray-500">Left behind on {new Date(selectedCart.createdAt).toLocaleString()}</p>
              </div>
              <button
                  ...
                  <div key={idx} className="flex gap-4 items-center p-3 bg-gray-50 rounded-xl border border-gray-100">
                    ...
                    <div className="flex-1">
                      ...
                      <p className="text-xs text-gray-500 mt-1 font-mono">
                        {item.variantId} {item.size && `• Size: ${item.size}`} {item.color && `• Color: ${item.color}`}
                      </p>
                      <div className="flex justify-between items-center mt-2">
                        <span className="text-xs font-bold bg-white border border-gray-200 px-2 py-0.5 rounded text-gray-600">Qty: {item.quantity}</span>
                        <span className="font-black text-indigo-600">৳{Number(item.price).toLocaleString()}</span>
                      </div>
                    </div>
                  </div>
                ))
              ) : (
                <p className="text-center text-sm text-gray-500 py-8">No specific item data found.</p>
              )}
            </div>
            ...
            <div className="p-5 border-t border-gray-100 bg-gray-50 flex items-center justify-between">
              ...
              <span className="text-2xl font-black text-gray-900">৳{Number(selectedCart.totalAmount).toLocaleString()}</span>
            </div>
          </div>
        </div>
      )}
