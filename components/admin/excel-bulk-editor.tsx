...
export function ExcelBulkEditor({ initialProducts }: { initialProducts: Product[] }) {
  ...
  const handleVariantChange = (productId: string, variantId: string, field: keyof ProductVariant, value: any) => {
    setProducts(prev => prev.map(p => {
  ...
  const handleSave = async () => {
    ...
    const vUpdates = products.flatMap(p => p.variants.map(v => ({
      id: v.id,
      stock: v.stock,
      price: v.price,
      sku: v.sku
    ...
    <div className="bg-white border border-gray-200 rounded-xl shadow-sm overflow-hidden flex flex-col">
      <div className="flex items-center justify-between p-4 bg-gray-50 border-b border-gray-200">
        <div className="flex items-center gap-3">
          <span className="text-sm font-semibold text-gray-700">Catalog Size: {products.length} Products</span>
          {changedCount > 0 && (
        ...
        <table className="w-full text-left text-sm whitespace-nowrap">
          <thead>
            <tr className="bg-gray-100/50 text-gray-500 font-medium uppercase tracking-wider text-[11px] border-b border-gray-200">
              <th className="px-4 py-3 sticky left-0 bg-gray-100/90 backdrop-blur z-10 w-[250px]">Product / Variant Name</th>
            ...
            {products.map(product => (
              <React.Fragment key={product.id}>
                ...
                <tr className="bg-gray-50/30 hover:bg-indigo-50/30 transition-colors group">
                  ...
                  <td className="px-4 py-3 font-sans text-gray-500">{product.category?.name || "—"}</td>
                  ...
                  <td className="px-4 py-3">
                    <input 
                      ...
                      onChange={(e) => handleProductChange(product.id, 'basePrice', Number(e.target.value))}
                  ...
                  <tr key={variant.id} className="hover:bg-blue-50/50 transition-colors group">
                    ...
                    <td className="px-4 py-2">
                      <input 
                        type="text" 
                        value={variant.sku}
                        onChange={(e) => handleVariantChange(product.id, variant.id, 'sku', e.target.value)}
